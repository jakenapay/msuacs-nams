$(document).ready(function() {
    $('#officesTable').DataTable({
        ...
        ajax: {
            url: site_url + 'admin/configurations/offices/list', // Server-side data endpoint
            type: 'GET',
            dataType: 'json',
        ...
        buttons: [
            {
                ...
                title: 'Offices-CSV-' + getFormattedDate() 
            },
            {
                extend: 'excelHtml5',
                ...
                title: 'Offices-Excel-' + getFormattedDate()
            },
        ],
    });
    ...
    $('#officesTable').on('click', '.edit-btn', function() {
        var id = $(this).data('id');
        ...
        $.ajax({
            url: site_url + 'admin/configurations/offices/edit/view/' + id,
            type: 'GET',
...
/****************************DELETE FUNCTION**************************************** */    
    ...
    $('#officesTable').on('click', '.delete-btn', function(e) {
        e.preventDefault();
        var id = $(this).data("id");
        iziToast.question({
            message: "Delete Item?",
            ...
            buttons: [
                ["<button class=\"text-white\">Confirm</button>", function(instance, toast, button, e, inputs) {
                    $.ajax({
                        url: site_url + "admin/configurations/offices/delete/" + id,
                        type: "POST",
