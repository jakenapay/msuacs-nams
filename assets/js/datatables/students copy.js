$(document).ready(function() {
    ...
    $('#studentsTable').DataTable({
        ...
        ajax: {
            url: site_url + 'admin/user_management/students/list',
            type: 'GET',
            dataType: 'json',
        },
        autoWidth: false,
        ...
        buttons: [
            {
                ...
                title: 'Students_CSV-' + getFormattedDate()  // Add a custom file name for CSV
            },
            {
                extend: 'excelHtml5',
                ...
                title: 'Students_Excel-' + getFormattedDate()  // Add a custom file name for Excel
            },
        ],
...
/****************************IZIMODAL**************************************** */    
    ...
    $('#studentsTable').on('click', '.edit-btn', function() {
        ...
        $.ajax({
            url: site_url + 'admin/user_management/students/view/' + id,
            type: 'GET',
        ...
        iziToast.question({
            ...
            buttons: [
                ["<button class=\"text-white\">Confirm</button>", function(instance, toast, button, e, inputs) {
                    $.ajax({
                        url: site_url + "admin/user_management/students/delete/" + id,
                        type: "POST",
...
/****************************BAN FUNCTION**************************************** */    
    ...
    // Handle delete button click
    $('#studentsTable').on('click', '.ban-btn', function(e) {
        e.preventDefault();
        var id = $(this).data("id");
            ...
            success: function(response) {
                if (response.status === 200) {
                    ...
                    iziToast.question({
                        ...
                        zindex: 1031,
                        inputs: [
                            ["<input type=\"password\" id=\"dashboard-password\" placeholder=\"Enter password\" class=\"form-control text-white\">"],
                            [
                                `<select id="dashboard-reason" class="form-control" style="color: white;" required>
                                    <option style="color: black;" value="" disabled selected>Choose reason of ban</option>
                            ...
                            [
                                `<select id="dashboard-location" class="form-control" style="color: white;" multiple>
                                    ${locations.map(location => `<option style="color: white;" value="${location.id}">${location.name}</option>`).join('')}
                                </select>`
                            ]
                        ],
                        buttons: [
                            ["<button class=\"text-white\">Confirm</button>", function(instance, toast, button, e, inputs) {
                                ...
                                var selectedLocations = $("#dashboard-location").val();
                                var locationsNameString = selectedLocations.map(id => locations.find(location => location.id === id).name).join(', ');
                                var locationsString = selectedLocations.join(', ');
                                if (reason === "Other") {
                                ...
                                $.ajax({
                                    url: site_url + "admin/user_management/students/ban/" + id,
                                    type: "POST",
                                    data: {
                                        id: id,
                                        ...
                                        locationNames: locationsNameString
                                    },
                                    beforeSend: function() {
                                        instance.hide({
                                            transitionOut: "fadeOut"
    ...
    // Handle delete button click
    $('#studentsTable').on('click', '.unban-btn', function(e) {
        e.preventDefault();
        var id = $(this).data("id");
        iziToast.question({
            message: "Unban student?",
            ...
            inputs: [
                ...
                [
                    `<select id="unban-reason" class="form-control text-white">
                        <option style="color: black;" value="" selected disabled hidden>Reason for unbanning</option>
            ...
            buttons: [
                ["<button class=\"text-white\">Confirm</button>", function(instance, toast, button, e, inputs) {
                    ...
                    $.ajax({
                        url: site_url + "admin/user_management/students/unban/" + id,
                        type: "POST",
                        data: "id=" + id + "&password=" + inputs[0].value + "&reason=" + reason,
