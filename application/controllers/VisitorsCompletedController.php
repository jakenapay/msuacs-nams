...
class VisitorsCompletedController extends CI_Controller {
    ...
    public function completedList(){
        ...
        $totalRecords = $this->db->count_all_results('visitors_completed');
    ...
    public function viewVisitor($id) {
        $visitor = $this->db->get_where('visitors_completed', ['id' => $id])->row_array();
        if ($visitor) {
            $this->load->view('modals/view_completed_visitor', ['visitor' => $visitor]);
        } else {
            echo json_encode([
                'status' => 400,
    ...
    private function deleteVisitor($id){
        ...
        $old_image = $this->VisitorsCompletedModel->getImage($id);
        if ($old_image && file_exists('./'. $old_image)) {
            unlink('./'. $old_image);
        }

        $this->VisitorsCompletedModel->delete($id);
        $rows = $this->db->affected_rows();

        if ($rows > 0) {
            $this->AuthModel->insertSecurity(
                $this->session->userdata('admin')['id'], 
                "Deleted an Completed Visitor with ID: " . $id, 
