...
class VisitorsCards extends CI_Controller {
    ...
    public function index(){
        ...
        $d['title'] = 'Visitors Card Management';
        ...
        $this->load->view('admin/visit_management/visitors_card', $d);
    ...
    private function banVisitor($id){
        $this->VisitorsApproveModel->banVisitor($id);
        ...
        if ($rows > 0) {
            $this->AuthModel->insertSecurity(
                $this->session->userdata('admin')['id'], 
                "Banned a active visitor with RFID# " . $rfid, 
                $this->session->userdata('admin')['username'], 
            );
    ...
    private function unbanVisitor($id){
        $this->VisitorsApproveModel->unbanVisitor($id);
        $rfid = $this->VisitorsApproveModel->getRFID($id);
        $rows = $this->db->affected_rows();
        ...
        if ($rows > 0) {
            $this->AuthModel->insertSecurity(
                $this->session->userdata('admin')['id'], 
                "Unbanned a Visitor with RFID# " . $rfid, 
                $this->session->userdata('admin')['username'], 
            );
