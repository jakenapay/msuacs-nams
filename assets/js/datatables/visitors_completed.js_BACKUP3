$(document).ready(function() {
    //DATATABLES
    $('#visitorsCompletedTable').DataTable({
        responsive: true,
        serverSide: true, // server-side processing
        dom: '<"row"<"col-sm-4"l><"col-sm-4 text-center"B><"col-sm-4"f>>rtip',
        ajax: {
            url: site_url + 'admin/visit_management/visitors_concluded/list',
            type: 'GET',
            dataType: 'json',
        },
        autoWidth: false,
        ...
        columns: [
            ...
            { 
                ...
                orderable: false,
                render: function(data, type, row, meta) {
                    let status = data === '1'? `<span class="badge badge-warning">Pending</span>` : data === '2' ? `<span class="badge badge-success">Active</span>` : data == '4' ? `<span class="badge badge-success">Completed</span>` : `<span class="badge badge-warning">Banned</span>`;
                    return status;
                }
            },
            {
        ...
        buttons: [
            {
                ...
                title: 'VisitorsCompleted_CSV-' + getFormattedDate()  // Add a custom file name for CSV
            },
            {
                extend: 'excelHtml5',
                ...
                title: 'VisitorsCompleted_Excel-' + getFormattedDate()  // Add a custom file name for Excel
            },
        ],
...
/****************************IZIMODAL**************************************** */    
    ...
    $('#visitorsCompletedTable').on('click', '.edit-btn', function() {
        var id = $(this).data('id');
        ...
        $.ajax({
            url: site_url + 'admin/visit_management/visitors_concluded/view/' + id,
            type: 'GET',
...
/****************************DELETE FUNCTION**************************************** */    
    ...
    $('#visitorsCompletedTable').on('click', '.delete-btn', function(e) {
        e.preventDefault();
        var id = $(this).data("id");
        iziToast.question({
            message: "Delete Item?",
            ...
            buttons: [
                ["<button class=\"text-white\">Confirm</button>", function(instance, toast, button, e, inputs) {
                    $.ajax({
                        url: site_url + "admin/visit_management/visitors_concluded/delete/" + id,
                        type: "POST",
                        ...
                        success: function(result) {
                            ...
                            if (response.status == 200) {
                                $('#visitorsCompletedTable').DataTable().ajax.reload();
