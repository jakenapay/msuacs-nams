    ...
    <div class="row">
        <div class="col-sm-12 dashboard-btn" id="add-row">
            <a id="add-btn" class="btn btn-sm btn-icon-split mb-4 shadow-sm">
                ...
                <span class="text text-white d-none d-sm-block">Add New Faculty</span>
            </a>
    ...
    <div class="shadow mb-4 mt-2">
        <div class="card-body">
            <div class="table-responsive-sm">
                <table class="table table-striped" id="facultyTable" width="100%" cellspacing="0"></table>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#add-row').on('click', '#add-btn', function () {
            $.ajax({
                url: site_url + 'admin/user_management/faculty/add/',
                type: 'GET',
        ...
        $(document).on('submit', '#addFacultyForm', function (e) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (serverResponse.status == 200) {
                        console.log('Success:', serverResponse.message); // Log success message
                        ...
                        console.log('Error Details:', errors); // Log error details

                        for (let key in errors) {
                            if (errors.hasOwnProperty(key)) {
                                console.log('Error Key:', key, 'Error Message:', errors[key]); // Log individual error keys and messages
                                danger(errors[key], "fa fa-exclamation-triangle");
                            } else {
                                console.log('General Error:', errors); // Log general errors
        ...
        $("#modal-excel-import").iziModal({
            title: 'Import New Faculty with Excel',
            subtitle: 'Add New Faculty Record via Import',
            ...
            onOpening: function (modal) {
                modal.startLoading();
                $.get(site_url + 'admin/user_management/faculty/excel_import_form', function (data) {
    ...
    $("#modal-data-import").iziModal({
        ...
        subtitle: 'Import Faculty Data from SAIS',
        ...
        <div class="text-center mb-4">
            ...
            <h5>Are you sure you want to import faculty data from SAIS?</h5>
            <p class="text-muted">This will sync the latest faculty information.</p>
        </div>
    ...
    $(document).on('click', '#confirmDataImport', function () {
        $.ajax({
            url: site_url + 'api/getAllData/faculty',
            type: 'GET',
            beforeSend: function () {
